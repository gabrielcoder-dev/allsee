...
export default function FilterModal({ open, onClose, onFilter }: { open: boolean, onClose: () => void, onFilter?: (tipo: string | null) => void }) {
  ...
  const toggleTipo = (tipo: string) => {
    setSelectedTipo(prev => (prev.includes(tipo) ? [] : [tipo]))
  }
    ...
    <div className="fixed inset-0 z-[9999] flex">
      ...
      >
        <div className="flex items-center justify-between px-6 py-5">
          <span className="text-2xl font-semibold">
            ...
            <span className="text-base font-normal text-gray-500">
              {selectedTipo.length > 0 ? `(${selectedTipo.length})` : ''}
            </span>
          </span>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t">
          <button className="text-gray-600 text-sm" onClick={limparFiltro} type="button">
            limpar filtro
          </button>
          <button
          ...
          >
            ...
            <svg width="18" height="18" fill="none" viewBox="0 0 24 24">
              <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M7 12h10m-4 6h4" />
            </svg>
          </button>
        </div>
      </div>
