...
const SimpleMap = dynamic(() => import('./SimpleMap'), { 
  ssr: false,
  loading: () => (
    <div className="hidden xl:flex w-[420px] 2xl:w-[520px] flex-shrink-0 z-0 items-center justify-center map-loading" style={{ height: '100vh' }}>
...
export default function MapboxWrapper({ anunciosFiltrados, onCityFound, userNicho, specificTotemId, isFullscreen = false, onToggleMapView, isInitialLoading = false }: MapboxWrapperProps) {
  return (
    <Suspense fallback={
      <div className={`${isFullscreen ? 'w-full' : 'hidden xl:flex w-[420px] 2xl:w-[520px]'} flex-shrink-0 z-0 items-center justify-center map-loading`} style={{ height: '100vh' }}>
        {!isInitialLoading && (
          <div className="flex flex-col items-center gap-2">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-orange-500"></div>
            <span className="text-sm text-gray-600">Carregando mapa...</span>
