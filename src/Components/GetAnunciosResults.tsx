...
export default function GetAnunciosResults({
  ...
  bairros,
  orderBy,
  onChangeAnunciosFiltrados,
  userNicho,
  onSpecificTotemFound,
  specificTotemId,
  isInitialLoading = false,
  selectedCity,
}: GetAnunciosResultsProps) {
  const {
    adicionarProduto,
    removerProduto,
    produtos,
  ...
  const reorderWithSpecificTotem = (
    anuncios: Anuncio[],
    specificTotemId: number
  ) => {
    const specificTotem = anuncios.find(
      (anuncio) => anuncio.id === specificTotemId
    );
    const otherAnuncios = anuncios.filter(
      (anuncio) => anuncio.id !== specificTotemId
    );
    ...
    async function fetchAnuncios() {
      ...
      if (selectedCity) {
        ...
        try {
          const response = await fetch(
            `/api/anuncios-por-cidade?cityLat=${selectedCity.lat}&cityLng=${
              selectedCity.lng
            }&cityName=${encodeURIComponent(selectedCity.name)}&radius=50`
          );
          const cityData = await response.json();

          console.log(
            "📊 Totens encontrados na cidade:",
            cityData.totens.length
          );
          ...
          let filteredTotens = cityData.totens.filter(
            (totem: any) => totem[durationColumn]
          );

          // Filtrar por tipo de mídia se especificado
          if (tipoMidia) {
            filteredTotens = filteredTotens.filter(
              (totem: any) => totem.type_screen === tipoMidia
            );
          }
          ...
          if (userNicho && userNicho !== "outro") {
            filteredTotens = filteredTotens.filter(
              (totem: any) => totem.nicho !== userNicho
            );
          }

          console.log(
            "✅ Totens filtrados para cidade:",
            filteredTotens.length
          );
          ...
          // Aplicar ordenação
          const anunciosOrdenados = ordenarAnuncios(
            filteredTotens,
            orderBy || ""
          );
      ...
      if (!error) {
        ...
        if (userNicho && userNicho !== "outro") {
          ...
          filteredData = anunciosData.filter(
            (anuncio: any) => anuncio.nicho !== userNicho
          );
        }
        ...
        if (bairros && bairros.length > 0) {
          ...
          console.log(
            "🔍 Total de anúncios antes do filtro:",
            filteredData.length
          );
          ...
          if (specificTotemId) {
            specificTotem = filteredData.find(
              (anuncio) => anuncio.id === specificTotemId
            );
            console.log(
              "🎯 Totem específico encontrado:",
              specificTotem ? specificTotem.name : "não encontrado"
            );
          }
          ...
          const filteredByAddress = filteredData.filter((anuncio: any) =>
            bairros.some((bairro) => {
              ...
              const addressParts = anuncioAddress
                .split(/[,\s-]+/)
                .filter((part: string) => part.length > 2);
              const hasMatchingPart = addressParts.some((part: string) =>
              ...
              const hasCommonKeyword = commonKeywords.some(
                (keyword) =>
                  anuncioAddress.includes(keyword) &&
                  searchTerm.includes(keyword)
              );

              const matches =
                containsTerm || hasMatchingPart || hasCommonKeyword;
          ...
          console.log(
            "🔍 Anúncios após filtro por endereço:",
            filteredByAddress.length
          );
          ...
          if (specificTotem) {
            ...
            const otherFilteredTotems = filteredByAddress.filter(
              (a) => a.id !== specificTotemId
            );
            filteredData = [specificTotem, ...otherFilteredTotems];
            console.log(
              "🎯 Totem específico colocado primeiro:",
              specificTotem.name
            );
            console.log(
              "🎯 Outros totens filtrados:",
              otherFilteredTotems.map((a) => a.name)
            );
            console.log(
              "🎯 Resultado final com totem específico primeiro:",
              filteredData.map((a) => ({
                id: a.id,
                name: a.name,
                address: a.address,
            ...
            console.log(
              "⚠️ Totem específico não encontrado, usando apenas filtro por endereço"
            );
          }
        } else if (specificTotemId) {
          // Se não há filtro por bairros mas há totem específico, mostrar todos os totens
          console.log(
            "🎯 Totem específico selecionado sem filtro de bairros - mostrando todos os totens"
          );
        }
        ...
        if (specificTotemId && anunciosOrdenados.length > 0) {
          const specificTotemIndex = anunciosOrdenados.findIndex(
            (a) => a.id === specificTotemId
          );
          if (specificTotemIndex > 0) {
            console.log(
              "🔄 Recolocando totem específico na primeira posição após ordenação"
            );
            const specificTotem = anunciosOrdenados[specificTotemIndex];
            const otherAnuncios = anunciosOrdenados.filter(
              (a) => a.id !== specificTotemId
            );
            anunciosOrdenados = [specificTotem, ...otherAnuncios];
            console.log(
              "🎯 Ordem final após correção:",
              anunciosOrdenados.map((a) => ({ id: a.id, name: a.name }))
            );
          }
        }
          ...
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-6 max-w-md">
              ...
              <p className="text-orange-700 mb-4">
                Não há totens disponíveis para o seu nicho ({userNicho}). Os
                totens do mesmo nicho não são exibidos para evitar concorrência.
              </p>
  ...
  return (
    <>
      ...
      {modalImage && (
        <ImageModal
          imageUrl={modalImage.image}
          name={modalImage.name}
          address={modalImage.address}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 p-4 pb-32 md:pb-12">
          {anuncios.map((anuncio) => {
            ...
            return (
              ...
              >
                <div
                  className="rounded-lg overflow-hidden h-48 flex items-center justify-center bg-gray-100 mb-2 cursor-pointer relative group"
                  onClick={() =>
                    setModalImage({
                      image: anuncio.image,
                      name: anuncio.name,
                ...
                >
                  ...
                  {anuncio.screen_type && (
                    <div className="absolute flex items-center gap-1 top-2 right-2 bg-orange-500/90 backdrop-blur-sm rounded-lg p-1.5 shadow-md">
                      <Smartphone
                      ...
                      <span className="text-white text-xs font-medium">
                        {anuncio.screen_type === "down" ? "Deitado" : "Em pé"}
                      </span>
                    </div>
                  )}
                  {/* Overlay de hover */}
                ...
                <div className="flex gap-2 mb-2">
                  {anuncio.type_screen?.toLowerCase() === "impresso" ? (
                    <span className="bg-green-600 text-white text-xs xl:text-sm 2xl:text-base px-2 py-1 rounded font-medium flex items-center gap-1">
                      <Printer className="w-4 h-4 xl:w-5 xl:h-5 2xl:w-6 2xl:h-6 mr-1 inline" /> impresso
                    </span>
                  ) : (
                    <span className="bg-purple-600 text-white text-xs xl:text-sm 2xl:text-base px-2 py-1 rounded font-medium flex items-center gap-1">
                      <Monitor className="w-4 h-4 xl:w-5 xl:h-5 2xl:w-6 2xl:h-6 mr-1 inline" /> digital
                    </span>
                  )}
                </div>
                <h3 className="font-bold text-lg">{anuncio.name}</h3>
                <div className="text-gray-500 text-xs xl:text-sm 2xl:text-base mb-1">
                  {anuncio.address}
                </div>
                <div className="flex gap-4 mb-1">
                  <div className="flex flex-col items-start">
                    <span className="text-[9px] xl:text-xs 2xl:text-sm text-gray-500 font-medium lowercase flex items-center gap-1">
                      ...
                      <span className="text-[9px] xl:text-sm 2xl:text-base">
                        <PlayIcon className="w-2.5 xl:w-3.5 2xl:w-4" />
                      </span>
                    </span>
                    <span className="font-bold text-sm xl:text-lg 2xl:text-xl">
                      {String(anuncio.display) === "fixo"
                        ? "fixo"
                        : anuncio.type_screen === "digital"
                        ? formatarMilhar(displayCalculado)
                        : formatarMilhar(
                            Number(anuncio.display) &&
                              !isNaN(Number(anuncio.display))
                              ? Number(anuncio.display)
                              : 0
                          )}
                    </span>
                  ...
                  <div className="flex flex-col items-start">
                    <span className="text-[9px] xl:text-xs 2xl:text-sm text-gray-500 font-medium lowercase flex items-center gap-1">
                      alcance{" "}
                      <span className="text-[9px] xl:text-sm 2xl:text-base">
                        <User2 className="w-2.5 xl:w-3.5 2xl:w-4" />
                      </span>
                    </span>
                    <span className="font-bold text-sm xl:text-lg 2xl:text-xl">
                  ...
                  <div className="flex flex-col items-start">
                    <span className="text-[9px] xl:text-xs 2xl:text-sm text-gray-500 font-medium lowercase flex items-center gap-1">
                      impacto{" "}
                      <span className="text-[9px] xl:text-sm 2xl:text-base">
                        <Zap className="w-2.5 xl:w-3.5 2xl:w-4" />
                      </span>
                    </span>
                    <span className="font-bold text-sm xl:text-lg 2xl:text-xl">
                ...
                <div className="text-xs text-gray-800 mb-1 font-bold">
                  Telas: {anuncio.screens}
                </div>
                ...
                {(() => {
                  ...
                  if (durationsTrue > 1) {
                    if (selectedDuration === "4")
                      precoOriginal = anuncio.price * 2;
                    if (selectedDuration === "12")
                      precoOriginal = anuncio.price * 6;
                    if (selectedDuration === "24")
                      precoOriginal = anuncio.price * 12;
                  }
                  return (
                    <div className="mb-1 flex flex-col gap-1">
                      {precoOriginal !== precoCalculado && (
                        <span className="text-sm text-gray-400 line-through">
                          R${" "}
                          {Number(precoOriginal).toLocaleString("pt-BR", {
                      ...
                      <span className="text-lg font-bold text-green-700">
                        R${" "}
                        {Number(precoCalculado).toLocaleString("pt-BR", {
                          minimumFractionDigits: 2,
                        })}
                      </span>
                    </div>
                ...
                <div className="text-xs text-gray-500 mb-2">
                  / {selectedDuration} semana
                  {Number(selectedDuration) > 1 ? "s" : ""}
                </div>
                <button
                  className={`w-full cursor-pointer flex items-center justify-center gap-4 border rounded-lg py-2 text-base font-semibold transition ${
                    estaNoCarrinho
                ...
                >
                  {estaNoCarrinho ? (
                    <>
                      remover ponto{" "}
                      <TrashIcon className="inline w-4 h-4 mr-1" />
                    </>
                  ) : (
                    <>
                      adicionar ponto{" "}
                      <ShoppingCartIcon className="inline w-4 h-4 mr-1" />
                    </>
                  )}
                </button>
              </div>
